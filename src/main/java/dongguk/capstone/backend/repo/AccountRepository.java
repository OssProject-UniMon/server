...
@Repository
public interface AccountRepository extends JpaRepository<Account, AccountId> { // 나중에 변경 필요
    // 복합키로 pk를 설정한 엔티티의 경우 findById 같은 메소드를 사용하지 못하고(여러 개의 칼럼으로 pk를 구성해서 pk가 id 하나가 아니기 때문),
    // 따로 쿼리문과 find 메소드를 설정해주어야 한다.
    @Query("SELECT a FROM Account a WHERE a.accountId.userId = :userId AND a.accountId.bankAccountNum = :bankAccountNum")
